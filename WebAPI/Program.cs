    ...
    { 
        ...
        private static void ApplyMigrations(WebApplication app)
        {
            using (var scope = app.Services.CreateScope())
            {
                ...
                // Попробуем выполнить миграции
                try
                {
                    // Применяем только ожидающие миграции
                    db.Database.Migrate();
                }
                catch (Exception ex)
                {
                    // Здесь вы можете обработать ошибки по своему усмотрению,
                    // например, логгировать без остановки приложения
